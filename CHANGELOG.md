...
- `scripts/diff_utils.py` — pure utility module for diff splitting, file batching, verdict extraction, and review aggregation. Importable directly by tests.
...
- Post-comment step simplified: diff-size messaging is now generated by `review.py` so the comment accurately reflects whether truncation or chunking occurred.
